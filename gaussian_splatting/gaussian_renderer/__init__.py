...
def render(
    ...
    # print("seg shape", rendered_features.shape)
...
def render_seg(
    ...
    if colors_precomp is None:
        if pipe.convert_SHs_python:
            shs_view = pc.get_segmentation.transpose(1, 2).view(
                -1, 3, (pc.max_sh_degree + 1) ** 2
            )
            dir_pp = pc.get_xyz - viewpoint_camera.camera_center.repeat(
                pc.get_features.shape[0], 1
    ...
    if mask is not None:
        rendered_image, radii, depth, opacity = rasterizer(
            means3D=means3D[mask].detach().clone(),
            means2D=means2D[mask].detach().clone(),
            shs=shs[mask],
            colors_precomp=colors_precomp[mask] if colors_precomp is not None else None,
            opacities=opacity[mask].detach().clone(),
            scales=scales[mask].detach().clone(),
            rotations=rotations[mask].detach().clone(),
            cov3D_precomp=cov3D_precomp[mask] if cov3D_precomp is not None else None,
            theta=viewpoint_camera.cam_rot_delta.detach().clone(),
            rho=viewpoint_camera.cam_trans_delta.detach().clone(),
        )
    else:
        rendered_image, radii, depth, opacity, n_touched = rasterizer(
            means3D=means3D,
